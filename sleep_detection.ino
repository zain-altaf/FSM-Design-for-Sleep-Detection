...
{
  const int rows = (int)(1000/sample_interval);//with the current sample interval, it should equal 100
  bool one_minute_hr_data[rows] = {0}; //one column represents 1 second of data and we collect for 60 seconds
  bool one_minute_br_data[rows] = {0};
  bool decremented_hr = false;
  bool decremented_br = false;
  int total_br = 0;
  int total_hr = 0;

  for (int i = 0; i < t_int; i++)
  {
    Serial.println("Unprocessed array");
    for (int row = 0; row < rows; row++) 
    {
      //taking 100 samples each second
        ...
        //Logic to ensure double reading of peaks doesn't happen as we write over the old array
        
        if (row == rows-1)Serial.println(peak_pin_hr);
        else Serial.print(peak_pin_hr);
        ...
        if (row - 1>= 0 && one_minute_hr_data[row-1] == 0 && one_minute_hr_data[row] == 1) total_hr++;
        else if (row == 0 && i == 0 && one_minute_hr_data[row] == 1) total_hr++;
        
        if (row - 1>= 0 && one_minute_br_data[row-1] == 0 && one_minute_br_data[row] == 1) total_br++;
        else if (row == 0 && i == 0 && one_minute_br_data[row] == 1) total_br++;
